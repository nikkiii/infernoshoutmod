define(['https://cdn.socket.io/socket.io-1.2.0.js', 'vbutil', 'http://cdn.probablyaserver.com/libs/jquery-ui/jquery-ui.min.js'], function(io, vbutil) {
	...
	var WebSocketPlugin = function(mod) {
		...
		mod.addSetting('ismchat-enable', 'checkbox', function(val) {
			enabled = val;

			if (enabled) {
				initTab();
			} else {
				mod.removeTab('ismchat');
			}
		...
		mod.addSetting('ismchat-flash', 'checkbox', function(val) {
			flash = val;
		});
		...
		function initTab() {
			$tab = mod.addStaticTab('ismchat', 'ISM Chat', '', {
				shout_params : {
					prefix : '/ismchat '
				}
			});

			$chat = $tab.children('.content_box');
		}
